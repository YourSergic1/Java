...
public class GameServiceImplementation implements GameService {
    ...
    @Autowired
    GameServiceImplementation(RepositoryServiceImpl repositoryService) {
        this.repositoryServiceImpl = repositoryService;
    }

    @Override
    public int isGameOver(Game game, Principal principal) {
        int result;
        if (!game.isGameWithHuman()) result = isGameOverWithComputer(game);
        else result = isGameOverWithHuman(game, principal);
        return result;
    }

    public int isGameOverWithHuman(Game game, Principal principal) {
        int[][] board = game.getField().getBoard();
        UUID userId = repositoryServiceImpl.getUser(principal.getName()).get().getId();
    ...
    public boolean isPlayerTurn(Game game, Principal principal) {
        UUID userId = repositoryServiceImpl.getUser(principal.getName()).get().getId();
    ...
    public int playerFigure(Game game, Principal principal) {
        UUID userId = repositoryServiceImpl.getUser(principal.getName()).get().getId();
        boolean isUserPlayer1 = game.getUser1().equals(userId);
