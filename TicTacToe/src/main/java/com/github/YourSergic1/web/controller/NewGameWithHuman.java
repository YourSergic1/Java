...
public class NewGameWithHuman {
    ...
    @Autowired
    public NewGameWithHuman(RepositoryServiceImpl repositoryServiceImpl, GameService gameServiceImpl, UserService userServiceImpl, JwtProvider jwtProvider) {
    ...
    public String createGameWithHuman(@RequestParam int num, @CookieValue(name = "accessToken", required = false) String accessToken) {
        final Claims claims = jwtProvider.getClaims(accessToken);
        UUID userId = UUID.fromString(claims.get("id").toString());
        Game game;
        if (num == 1)
            game = new Game(0, 1, userId, null, true);
        else game = new Game(1, 0, userId, null, true);
