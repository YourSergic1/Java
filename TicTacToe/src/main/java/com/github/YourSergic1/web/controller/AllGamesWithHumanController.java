...
public class AllGamesWithHumanController {
    ...
    @Autowired
    public AllGamesWithHumanController(RepositoryServiceImpl repositoryServiceImpl, GameService gameService, JwtProvider jwtProvider) {
    ...
    public String allGamesWithHuman(@CookieValue(name = "accessToken", required = false) String accessToken, Model model) {
        final Claims claims = jwtProvider.getClaims(accessToken);
        String login = claims.getSubject();
        List<GameEntity> allGames = repositoryServiceImpl.getAllGamesWithHuman(login);
